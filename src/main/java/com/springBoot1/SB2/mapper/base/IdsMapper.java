...
public class IdsMapper {
    ...
    public Authority mapLongIdToAuthority(Long id) throws ApiException {
        if (id==null)return null;
        return authorityRepository.findById(id).orElseThrow(() -> new ApiException(messageSource.getMessage("exceptions.id.notfound", ArrayUtil.of(id.toString()), LocaleContextHolder.getLocale())));
    }
